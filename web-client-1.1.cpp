...
main(int argc, char *argv[])
{
  ...
  for(int i=0; i<n_url; i++){
    ...
    #ifdef DEBUG
    cerr<<i<<"th URL path is:"<<url[i].path<<" URL host is:"<<url[i].host<<" URL port is:"<<url[i].port<<endl;
    #endif
  }
  ...
  // get address
  cerr<<"host is "<<url[0].host<<" and port is "<<url[0].port<<endl;
  ...
  //test only
  #ifdef DEBUG
  cerr<<"IP address of "<<url[0].host<<" is "<<ipstr1<<endl;
  #endif
  //================================================
  ...
  #ifdef DEBUG
  cerr<<"sin_port works"<<endl;
  #endif
  serverAddr.sin_addr.s_addr = inet_addr(ipstr1);
  memset(serverAddr.sin_zero, '\0', sizeof(serverAddr.sin_zero));
  #ifdef DEBUG
  cerr<<"memset serveraddr works"<<endl;
  #endif
  ...
  cerr<<"send the requset message "<<endl;
  ...
  if(select(FD_SETSIZE, &active_fd_set, NULL, NULL, &timeout) == 0){
	  cerr<<"Resend the request message."<<endl;
	  for(int i=0; i<n_url; i++){
	  if (send(sockfd, testRequest[i].c_str(), strlen(testRequest[i].c_str()), 0) == -1) {
		perror("send");
		return 1;
...
for(int i=0; i<n_url; i++){
  ...
  while (!findSeparation) {    
	...
	FD_SET (sockfd, &active_fd_set);
	if(select(FD_SETSIZE, &active_fd_set, NULL, NULL, &timeout) == 0){
	  cerr<<"Time out due to incomplete response message."<<endl;
	  close(sockfd);
	  return 1;
	}
	*/
    ...
    if ((recvLen = recv(sockfd, recvBuf, BUFSIZE, MSG_DONTWAIT)) == -1) {
		...
		if(duration == 4){
			cerr<<"recvLen in first while loop: "<<recvLen<<endl;
			cerr<<"Time out due to incomplete response message (1st -1)."<<endl;
			close(sockfd);
			return 1;
		}
    }
		...
		if(duration == 4){
			cerr<<"recvLen in first while loop: "<<recvLen<<endl;
			cerr<<"Time out due to incomplete response message (1st 0)."<<endl;
			close(sockfd);
			return 1;
		}
	}
	...
	if(foundSeparation != string::npos){
		//cerr<<"recvString inside foundSeparation is "<<endl<<recvString<<endl;
		...
		cerr<<"cont_len is "<<cont_len<<endl;
		int clen = atoi(cont_len.c_str());
		cerr<<"clen is "<<clen<<endl;
		remaining = clen - bodyString.length();
		findSeparation = true;
		cerr<<"remaining is "<<remaining<<endl;
	}
	start = clock();
	}	
	//string::npos    
  ...
  while(remaining>0){
	...
	*/
    ...
    if ((recvLen = recv(sockfd, recvBuf, BUFSIZE, MSG_DONTWAIT)) == -1) {
		duration = (clock() - start) /((double) CLOCKS_PER_SEC);
		if(duration == 4){
			cerr<<"recvLen in second while loop: "<<recvLen<<endl;
			cerr<<"Time out due to incomplete response message (2nd -1)."<<endl;
			close(sockfd);
			return 1;
		}
    }
	...
	if(recvLen == 0){
		...
		if(duration == 4){
			cerr<<"recvLen in second while loop: "<<recvLen<<endl;
			cerr<<"Time out due to incomplete response message (2nd 0)."<<endl;
			close(sockfd);
			return 1;
		}
	}
...
//recvString = recvString.erase(found_cont_len_end+100);
  ...
  //cerr<<"Recv String is "<<recvString<<endl;
  //cerr<<"line 178"<<endl;
  cerr<<"recvString length is "<<recvString.length()<<endl;
  vector<string> SplitResponse = split_response(recvString);
  //cerr<<"line 180"<<endl;
  //cerr<<"Receive string is "<<recvString;
  cerr<<"vector size is "<<SplitResponse.size()<<endl;
  //}
  //cerr<<"line 184"<<endl;
...
for(int i=0; i<n_url; i++){
  ...
  cerr<<"content length of "<<i<<"th is "<<contentLen<<endl;
  unsigned long contentLEN = atoi(contentLen.c_str());
 unsigned long bodyLEN = Res.getPayload().length();
  if(contentLEN!=bodyLEN){
	  cerr<<"Incomplete Response with URL:"<<url[i].host<<url[i].path<<endl;
	  return 1;
  }
  
  string resStatus=Res.getstatus();
#ifdef DEBUG
  cerr<<"status is "<<resStatus<<endl;
#endif
  if(resStatus != "200"){
    cerr<<"Not a good request to URL:"<<url[i].host<<url[i].path<<endl;
    return 1;
  }
  else{
    ...
    if(outfile.is_open()){
	  ...
	  //cerr<<"content length "<<content.length();
	  //outfile << content;
